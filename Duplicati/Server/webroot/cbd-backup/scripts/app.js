...
$(document).ready(function() {
    ...
    APP_DATA.throttle = function(cfg, callback, errorhandler) {
        serverWithCallback(
            { action: 'set-server-options', 
			  HTTP_METHOD: 'POST', 
    ...
    APP_DATA.showThrottle = function() {
        $('#throttle-dialog').dialog('open');
    };
	...
	*/
    ...
    $.showPopupMenu = function(menu, anchor, offset) {
        ...
        if (left + menuwidth > $(document).outerWidth())
            left = $(document).outerWidth() - menuwidth;
        if (top + menuheight > $(document).outerHeight())
            top = $(document).outerHeight() - menuheight;
    ...
    $('#main-settings').click(function() {
        ...
        $.showPopupMenu($('#main-control-menu'), $('#main-settings'), {y: pos.top - 10});

        $('#main-control-menu').css({ left: pos.left + 34 + "px", right: "" });
    });
    ...
    $('#throttle-submit').click(function() {
        APP_DATA.throttle();
    });
    ...
    $(document).on('server-state-lost', function() {
        ...
        $('#connection-lost-dialog-text').text('CBD Backup is not running...');
    ...
    $(document).on('server-state-countdown', function(e, f) {
        ...
        $('#connection-lost-dialog-text').html('CBD Backup is not running, <br/>retrying connection in ' + s + ' seconds');
        var button = $('#connection-lost-dialog').parent().find('.ui-dialog-buttonpane').find('.ui-button').first();
        button.attr("disabled",false).removeClass( 'ui-button-disabled ui-state-disabled' );
    });
    ...
    $('#throttle-dialog').hide();

    $('#main-control-menu-about').click(function() { APP_DATA.showAbout(); })
    $('#main-control-menu-throttle').click(function() { APP_DATA.showThrottle(); })
    ...
    $('#backup-details-delete').click(function(e) {
        ...
        if (name && confirm('Really delete configuration for ' + name + '? Note: This will not immediately delete remote backup files, and will not delete local files.'))
    ...
    $('#throttle-dialog').dialog({
        ...
        buttons: [
            ...
            { text: 'submit', click: function(event, ui) {
			
				var maxuploadPerSecond = $('input[name=throttle-upload]:checked', '#throttle-dialog-form').val();
				var data = {};
				data['max-upload-speed'] = maxuploadPerSecond;
                ...
                APP_DATA.callServer(
                    ...
                    function(a,b,msg) {
                        alert('Failed to save throttle data: ' + msg);
                    }
                );
