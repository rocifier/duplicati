...
//    $('#backup-labels').change(updateState);
    ...
    $('#backup-destination-label').click(function() {
        $('#connection-uri-dialog').dialog('open');
        $('#connection-uri-dialog').trigger('setup-dialog', $('#backup-uri'));
    });
	$('#backup-uri').change(function(){
		if ( $('#backup-uri').val().lastIndexOf("file://", 0) === 0 ) {
			$('#backup-destination-label').text("Local storage media");
		} else {
			$('#backup-destination-label').text("The Cloud");
		}
	})
