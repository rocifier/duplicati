$(document).ready(function() {
    ...
    APP_DATA.plugins.backend['s3'] = {
        ...
        setup: function(dlg, div) {
            ...
            var email = EDIT_URI.createFieldset({label: 'Email', name: 's3-email', before: $('#server-options-label'), watermark: 'Enter the email address you used to sign up with here'});
			var regiondrop = EDIT_URI.createFieldset({label: 'Storage region', name: 's3-region', before: $('#server-options-label'), watermark: 'Click for a list of regions'});
        ...
        validate: function(dlg, values) {
            if (!EDIT_URI.validate_input(values, true))
                return;
            if (values['s3-region'] == '')
                return EDIT_URI.validation_error(this.regiondrop_field, 'You must select a nearby region to use');
			if (values['s3-email'] == '')
                return EDIT_URI.validation_error(this.email_field, 'You must enter an email address');
