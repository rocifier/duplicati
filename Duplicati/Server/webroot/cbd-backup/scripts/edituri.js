...
$(document).ready(function() {
    ...
    var resetform = function() {
        ...
        $('#server-username').watermark('');
        $('#server-password').watermark('');
    ...
    $('#connection-uri-dialog').on( "setup-dialog", function( event, el ) {
        ...
        APP_DATA.getServerConfig(function(serverdata) {
            ...
            for (var i in { 's3': 0 }) {
                if (BACKEND_STATE.module_lookup[i]) {
                    //used[i] = true;
                    drop.append($("<option></option>").attr("value", i).text(BACKEND_STATE.module_lookup[i].DisplayName));
                }
            }
            for(var i in {'file':0}) {
                if (BACKEND_STATE.module_lookup[i]) {
